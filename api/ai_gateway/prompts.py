...
2. Every item has: name (<=60 chars), category (clothing|footwear|toiletries|medications|electronics|documents|activity_gear|kids|comfort|misc), item_class (underwear|tops|bottoms|sleepwear|toiletry_kit|medication|charger|other), qty (integer 1-99 — real counts for consumables like medication days; the app clamps), reason (<=15 words citing concrete input data), confidence (0-1), source_signal (activity|duration|destination|season|history|weather), priority (high|normal|optional), and for clothing/footwear a style_tag (underwear|casual|smart_casual|formal|traditional|outerwear|footwear|athleisure|sleep).
...
8. Respect local dress norms: Gulf/Arab destinations may warrant traditional wear (dishdasha, ghutra) with style_tag "traditional" when the traveler context fits; business or conference trips get a "formal" set (suit, ties). Jackets, coats, and shawls take "outerwear". NEVER omit style_tag on clothing or footwear.
