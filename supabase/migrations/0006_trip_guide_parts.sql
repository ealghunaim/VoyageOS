-- 0006: two-phase, per-destination guide cache  — applied 2026-07-30
-- Progressive guide: phase 'a' (Know + Eat, fast/Haiku) and phase 'b'
-- (Play + Visit + Go, Sonnet) generate and cache independently, per
-- destination, so the first tab paints in seconds while the rest fills in
-- behind it, and multi-stop trips get an independent guide per stop.
-- One row per (trip, destination, phase); backend uses the service-role
-- key so RLS never blocks it, but the owner policy is here for
-- defense-in-depth like the other trip tables.
--
...
create table if not exists public.trip_guide_parts (
  ...
  primary key (trip_id, destination_id, phase)
);
